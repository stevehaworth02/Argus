...
def construct_bipolar_from_referential(raw: mne.io.BaseRaw, bipolar_pairs: List[str]) -> Tuple[np.ndarray, List[str], float]:
    """
    Construct bipolar montage from referential EEG data.
    
    Bipolar channel = electrode1 - electrode2
    Example: 'FP1-F7' = 'EEG FP1-REF' - 'EEG F7-REF'
    
    Args:
        raw: MNE raw object with referential channels
        bipolar_pairs: List of desired bipolar channel names (e.g., 'FP1-F7')
    
    Returns:
        data: Bipolar data array (n_bipolar, n_samples)
        channels: List of constructed bipolar channel names
        sfreq: Sampling frequency
    """
    available_channels = [ch.upper() for ch in raw.ch_names]
    sfreq = raw.info['sfreq']
    
    def find_referential_channel(electrode: str) -> Optional[str]:
        """Find referential channel for an electrode (e.g., 'FP1' -> 'EEG FP1-REF')."""
        electrode = electrode.upper().strip()
        ...
        for pattern in patterns:
            for avail in raw.ch_names:
                if avail.upper().strip() == pattern:
                    return avail
        return None
    ...
    for bipolar_name in bipolar_pairs:
        # Parse bipolar name (e.g., 'FP1-F7' -> 'FP1', 'F7')
        parts = bipolar_name.split('-')
        if len(parts) != 2:
            continue
        
        electrode1, electrode2 = parts[0].strip(), parts[1].strip()
        
        # Find referential channels
        ref1 = find_referential_channel(electrode1)
        ref2 = find_referential_channel(electrode2)
        
        if ref1 and ref2:
            # Construct bipolar: electrode1 - electrode2
            idx1 = raw.ch_names.index(ref1)
            idx2 = raw.ch_names.index(ref2)
            
            data1 = raw.get_data(picks=[idx1])[0]
            data2 = raw.get_data(picks=[idx2])[0]
            
            bipolar_signal = data1 - data2
            bipolar_data.append(bipolar_signal)
            bipolar_channels.append(bipolar_name)
    
    if len(bipolar_data) == 0:
        return None, None, None
    
    # Stack into array
    data_array = np.array(bipolar_data, dtype=np.float32)
    
    return data_array, bipolar_channels, sfreq


def load_edf_file(edf_path: str, target_channels: List[str] = None, debug=False) -> Tuple[np.ndarray, List[str], float]:
    """
    Load EEG data from EDF file using MNE.
    Handles both bipolar and referential montages.
    If file has referential montage, constructs bipolar channels.
    
    Args:
        edf_path: Path to .edf file
        target_channels: List of bipolar channels to load (e.g., 'FP1-F7')
    ...
    try:
        ...
        if debug:
            print(f"\n  DEBUG: File {os.path.basename(edf_path)}")
            print(f"  Available channels ({len(available_channels)}): {available_channels[:3]}...")
        
        # Check if this is referential montage (channels end with -REF)
        is_referential = any('-REF' in ch.upper() for ch in available_channels)
        
        if debug:
            print(f"  Montage type: {'REFERENTIAL' if is_referential else 'BIPOLAR'}")
        
        # If target channels specified
        if target_channels:
            if is_referential:
                # Construct bipolar channels from referential data
                data, constructed_channels, sfreq = construct_bipolar_from_referential(raw, target_channels)
                
                if data is None:
                    if debug:
                        print(f"  Could not construct bipolar channels")
                    return None, None, None
                
                if debug:
                    print(f"  Constructed {len(constructed_channels)}/{len(target_channels)} bipolar channels")
                
                # Pad missing bipolar channels with zeros
                if len(constructed_channels) < len(target_channels):
                    n_samples = data.shape[1]
                    full_data = np.zeros((len(target_channels), n_samples), dtype=np.float32)
                    
                    for i, target_ch in enumerate(target_channels):
                        if target_ch in constructed_channels:
                            idx = constructed_channels.index(target_ch)
                ...
                # Original logic for bipolar files (flexible matching)
                def find_match(target: str, available: List[str]) -> Optional[str]:
                ...
                for target_ch in target_channels:
                    match = find_match(target_ch, available_channels)
                    if match:
                        channel_map[target_ch] = match
                
                present_channels = list(channel_map.values())
                
                if debug:
                    print(f"  Matched {len(present_channels)}/{len(target_channels)} channels")
...
def preprocess_artifact_file(edf_path: str, csv_path: str, config=Config, debug=False) -> Optional[Dict]:
    """
    ...
    Args:
        ...
        debug: Print debug info about channel matching
    ...
    if data is None:
        if debug:
            print(f"  DEBUG: Could not load {os.path.basename(edf_path)} - no matching channels found")
        return None
    
    # CRITICAL: Ensure exactly 22 channels
    if data.shape[0] != len(config.STANDARD_CHANNELS):
        if debug:
            print(f"  DEBUG: {os.path.basename(edf_path)} has {data.shape[0]} channels, expected {len(config.STANDARD_CHANNELS)}")
        return None
...
def preprocess_dataset(montage: str = '01_tcp_ar',
    ...
    for batch_idx in range(num_batches):
        ...
        if len(batch_windows) > 0:
            # Just concatenate all windows - files have different durations so different window counts
            # Each window should be (22, 2000) which is already validated in preprocess_artifact_file
            try:
                batch_data = {
                ...
                print(f"  Window shapes: {[w.shape for w in batch_windows[:5]]}")
                continue
    ...
    if save_path and len(batch_files) > 0:
        ...
        for batch_file in batch_files:
            if isinstance(batch_file, str):
                # Load and immediately extract data, then close file
                with np.load(batch_file, allow_pickle=True) as data:
                    all_windows.append(data['windows'].copy())
                    all_labels.append(data['labels'].copy())
                    all_metadata.extend(data['metadata'].copy())
        ...
        # Clean up batch files (files are now closed)
        ...
        for batch_file in batch_files:
            if isinstance(batch_file, str) and os.path.exists(batch_file):
                try:
                    ...
                    print(f"  Warning: Could not delete {batch_file} (file in use)")
                except Exception as e:
